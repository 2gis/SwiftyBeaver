...
class SwiftyBeaverTests: XCTestCase {
    ...
    func testLogVerifiesIfShouldLogOnAllDestinations() {
        ...
        XCTAssertEqual(dest1.shouldLogToLevel, SwiftyBeaverLogger.Level.warning)
        XCTAssertEqual(dest2.shouldLogToLevel, SwiftyBeaverLogger.Level.warning)
    ...
    func testLogCallsAllDestinations() {
        ...
        XCTAssertEqual(dest1.didSendToLevel, SwiftyBeaverLogger.Level.warning)
        XCTAssertEqual(dest2.didSendToLevel, SwiftyBeaverLogger.Level.warning)
    ...
    func testDestinationIntegration() {
        ...
        XCTAssertEqual(log.countDestinations(), 2)
        console2.format = "$L: $M"
        console2.minLevel = SwiftyBeaverLogger.Level.debug
        ...
        console2.format = "$L: $M"
        file2.minLevel = SwiftyBeaverLogger.Level.debug
    ...
    func testVersionAndBuild() {
        XCTAssertGreaterThan(SwiftyBeaverLogger.version.length, 4)
        XCTAssertGreaterThan(SwiftyBeaverLogger.build, 500)
    }

    func testStripParams() {
        var f = "singleParam"
        XCTAssertEqual(SwiftyBeaverLogger.stripParams(function: f), "singleParam()")
        f = "logWithParamFunc(_:foo:hello:)"
        XCTAssertEqual(SwiftyBeaverLogger.stripParams(function: f), "logWithParamFunc()")
        f = "aFunc()"
        XCTAssertEqual(SwiftyBeaverLogger.stripParams(function: f), "aFunc()")
    }
...
private class MockDestination: BaseDestination {
    var didSendToLevel: SwiftyBeaverLogger.Level?
    ...
    override func send(_ level: SwiftyBeaverLogger.Level, msg: String, thread: String, file: String, function: String, line: Int, context: Any?) -> String? {
    ...
    var shouldLogToLevel: SwiftyBeaverLogger.Level?
    var shouldLogPath: String?
    var shouldLogFunction: String?
    var shouldLogMessage: String?
    override func shouldLevelBeLogged(_ level: SwiftyBeaverLogger.Level, path: String, function: String, message: String?) -> Bool {
